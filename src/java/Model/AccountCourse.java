...
//    }
    ...
    public static String getAccountRole(int acc_id, int course_id) {
     ...
     public static List<Account> getTeacherCourse(int course_id) {
        List<Account> listAccount = new ArrayList<>();
        Connection conn = ConnectionBuilder.getConnection();
        String sql = "select acc_id from account_course where course_id=? AND role =  \"TH\" AND status =  \"approved\"";
        PreparedStatement pstm;
        try {
            pstm = conn.prepareStatement(sql);
            pstm.setInt(1, course_id);
     ...
     public static List<Account> getStudentCourse(int course_id) {
        List<Account> listAccount = new ArrayList<>();
        Connection conn = ConnectionBuilder.getConnection();
        String sql = "select acc_id from account_course where course_id=? AND role =  \"ST\" AND status =  \"approved\"";
        PreparedStatement pstm;
        try {
            pstm = conn.prepareStatement(sql);
            pstm.setInt(1, course_id);
